...
const authenticateToken = (
  ...
  jwt.verify(
    ...
    (err: any, user: any) => {
      ...
      if (typeof user === "object" && user.id && user.role) {
        req.user = user;
        next();
      } else {
...
app.get("/api/fdp/pending/:role", authenticateToken, async (req, res) => {
  ...
  const roleSignatureMap: Record<string, string> = {
...
app.post("/api/fdp/:id/sign", authenticateToken, async (req, res) => {
  ...
  const signingRole: string = selectedRole || req.user.role;
  ...
  try {
    ...
    // ── Mapping rôle → champs de signature ────────────────────────────────
    const signatureFieldMap: Record<string, { sig: string; date: string }> = {
    ...
    // ── Interdire la re-signature ──────────────────────────────────────────
    if (fdp[fieldInfo.sig]) {
      return res.status(400).json({ message: "Cette signature a déjà été apposée sur cette fiche" });
    }

    // ── Cas spécial : DG (montant requis) ─────────────────────────────────
    if (signingRole === "directeur_general") {
      let amountValue: number | null = null;

      if (fdp.type_description !== "Expression de besoin") {
        if (!validatedAmount) {
          return res.status(400).json({ message: "Le montant validé est obligatoire pour le Directeur Général." });
        }
        const cleaned = validatedAmount.toString().replace(/[^0-9]/g, "");
        if (!cleaned) {
          return res.status(400).json({ message: "Format invalide. Exemple : 250000" });
        }
        amountValue = parseInt(cleaned, 10);
        if (isNaN(amountValue) || amountValue <= 0) {
          return res.status(400).json({ message: "Le montant doit être supérieur à 0 FCFA." });
        }
      } else {
        if (validatedAmount) {
          const cleaned = validatedAmount.toString().replace(/[^0-9]/g, "");
          if (cleaned) {
            ...
            if (!isNaN(parsed) && parsed > 0) amountValue = parsed;
          }
        }
      }
...
function getNextStatus(signingRole: string, currentStatus: string): string {
  const statusOrder = [
    "pending_superieur_hierarchique",
...
app.listen(5001, "0.0.0.0", () => {
  console.log(`Server running on http://197.159.195.207:5000`);
});